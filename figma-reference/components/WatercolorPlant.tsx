...
class Flower {
  ...
  constructor(x: number, y: number, type: number, scale: number) {
    ...
    this.altColor =
      PALETTE.flowers[(this.colorBias + 1 + ((Math.random() * 2) | 0)) % PALETTE.flowers.length];
    this.darkColor = {
      h: this.baseColor.h,
  ...
  useEffect(() => {
    ...
    function loop() {
      ...
      for (let i = 0; i < stems.length; i++) {
        ...
        if (!done) allStemsDone = false;
          ...
          if (stem.segments.length > 16 && Math.random() > 0.62) {
            ...
            const budY =
              node.y + node.cos * side * (2 + Math.random() * 4) - (2 + Math.random() * 6);
            flowers.push(
              new Flower(
                budX,
                budY,
      ...
      for (let i = 0; i < flowers.length; i++) {
        flowers[i].draw(brush);
        if (flowers[i].age < flowers[i].maxAge) allFlowersDone = false;
      }
