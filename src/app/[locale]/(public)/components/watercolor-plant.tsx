...
const STEM_COLORS: [HSL, HSL, HSL] = [
  { h: 140, s: 30, l: 35 },
  { h: 130, s: 25, l: 40 },
  { h: 145, s: 20, l: 30 },
];

const FLOWER_COLORS: HSL[] = [
  { h: 348, s: 72, l: 58 },
  { h: 332, s: 64, l: 64 },
  { h: 12, s: 72, l: 62 },
  { h: 286, s: 40, l: 58 },
...
const CALYX_COLOR: HSL = { h: 132, s: 34, l: 30 };
const VASE_COLOR: HSL = { h: 0, s: 20, l: 92 };
const HEARTS_COLOR: HSL = { h: 350, s: 100, l: 65 };
...
function flowerColor(index: number): HSL {
  return FLOWER_COLORS[index % FLOWER_COLORS.length]!;
}
...
class Stem {
  ...
  leavesBySegment: Map<number, LeafData[]>;
  leafDensity: number;
  ...
  constructor(x: number, y: number, height: number, angle: number) {
    ...
    this.done = false;
    this.leavesBySegment = new Map();
    this.leafDensity = 0.14 + Math.random() * 0.08;
    this.hasFlower = false;
    this.flowerTypeHint = 0;
    ...
    for (let i = 0; i < totalSteps; i++) {
      ...
      if (i > totalSteps * 0.18 && i < totalSteps * 0.92 && Math.random() < this.leafDensity) {
        ...
        const existing = this.leavesBySegment.get(i);
        if (existing) {
          existing.push(leaf);
        } else {
          this.leavesBySegment.set(i, [leaf]);
        }
      }
      cx = nx;
      cy = ny;
  ...
  draw(brush: Brush) {
    ...
    for (let i = startIdx; i < drawEnd; i++) {
      ...
      brush.stroke(prev.x, prev.y, curr.x, curr.y, STEM_COLORS[1], width, 0.15);
      brush.stroke(prev.x, prev.y, curr.x, curr.y, STEM_COLORS[2], width * 0.55, 0.18);

      const leaves = this.leavesBySegment.get(i);
      if (leaves) {
  ...
  drawLeaf(brush: Brush, x: number, y: number, leaf: LeafData, stemWidth: number) {
    ...
    brush.blob(midX, midY, leaf.radius * 1.15, STEM_COLORS[0], 0.11, leaf.angle, 2.15);
    brush.blob(
      midX + nx * lobeOffset,
      midY + ny * lobeOffset,
      leaf.radius * 0.82,
    ...
    brush.stroke(rootX, rootY, tipX, tipY, STEM_COLORS[2], Math.max(0.45, stemWidth * 0.26), 0.15);
    brush.stroke(midX, midY, tipX, tipY, STEM_COLORS[2], 0.3, 0.08);
  }
}
...
class Flower {
  ...
  constructor(x: number, y: number, type: number, scale: number) {
    ...
    this.petals = [];
    this.centerDots = [];
    const colorBias = Math.floor(Math.random() * FLOWER_COLORS.length);
    this.baseColor = flowerColor(colorBias);
    this.altColor = flowerColor(colorBias + 1 + ((Math.random() * 2) | 0));
    this.darkColor = {
      h: this.baseColor.h,
  ...
  addPetal(angle: number, distance: number, radius: number, layer: number, stretch: number) {
    this.petals.push({
      angle,
      cos: Math.cos(angle),
      sin: Math.sin(angle),
  ...
  initShape() {
    if (this.type === 0) {
      ...
      this.addCenterDots(5, 1, 4.8, 1.2, 2.4, HEARTS_COLOR);
    } else if (this.type === 2) {
    ...
    if (this.age > 3) {
      ...
      if (this.capMode === "cluster") {
        ...
        for (let i = 0; i < this.capLobes; i++) {
          ...
          const cx =
            headX +
            (t - 0.5) * capWidth * spread +
            tiltShift +
            (Math.random() - 0.5) * capWidth * 0.08;
          const cy =
            headY -
            Math.sin(t * Math.PI) * capHeight +
            this.headTilt * (t - 0.5) * capHeight * 0.2 +
            (Math.random() - 0.5) * capHeight * 0.12;
          const c = Math.random() > 0.6 ? this.lightColor : this.baseColor;
          brush.blob(
            cx,
            cy,
            capHeight * (0.42 + Math.random() * 0.26),
            c,
            0.034,
            this.headTilt * 0.35,
            1.92,
...
export function WatercolorPlant() {
  const displayCanvasRef = useRef<HTMLCanvasElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  const hasStarted = useRef(false);

  useEffect(() => {
    const rawCanvas = displayCanvasRef.current;
    const rawContainer = containerRef.current;
    if (!rawCanvas || !rawContainer) return;

    const rawDisplayCtx = rawCanvas.getContext("2d", { alpha: true });
    if (!rawDisplayCtx) return;

    const paintCanvas = document.createElement("canvas");
    const rawPaintCtx = paintCanvas.getContext("2d", { alpha: true });
    if (!rawPaintCtx) return;

    // Re-assign to satisfy TypeScript closure narrowing
    const displayCanvas: HTMLCanvasElement = rawCanvas;
    const container: HTMLDivElement = rawContainer;
    const displayCtx: CanvasRenderingContext2D = rawDisplayCtx;
    const paintCtx: CanvasRenderingContext2D = rawPaintCtx;
    ...
    const COMPOSITE_FILTER = "url(#watercolor-plant) blur(0.5px)";
    ...
    function init() {
      ...
      const mouthHalf = VASE_WIDTH * VASE_MOUTH_RATIO * (width < 600 ? 0.95 : 1.05);
      ...
      for (let i = 0; i < count; i++) {
        ...
        const h = height * 0.16 + Math.random() * (height * 0.12);
        const stem = new Stem(baseX, startY, h, angle);
        stem.flowerTypeHint =
          Math.random() > 0.82 ? pickFlowerType() : typeOrder[i % FLOWER_TYPE_COUNT]!;
        stems.push(stem);
      }
      loop();
    }
    ...
    function loop() {
      ...
      for (const stem of stems) {
        ...
        if (!done) allStemsDone = false;
          ...
          flowers.push(
            new Flower(anchor.x, anchor.y, stem.flowerTypeHint, 0.64 + Math.random() * 0.17),
          );
          if (stem.segments.length > 16 && Math.random() > 0.62) {
            ...
            const nodeSide = Math.random() < 0.5 ? -1 : 1;
            const budX = node.x + -node.sin * nodeSide * (8 + Math.random() * 8);
            const budY =
              node.y + node.cos * nodeSide * (2 + Math.random() * 4) - (2 + Math.random() * 6);
            flowers.push(
              new Flower(
                budX,
                budY,
